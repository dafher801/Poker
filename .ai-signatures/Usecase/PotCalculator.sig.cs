...
namespace TexasHoldem.Usecase
{
    public static class PotCalculator
    {
        public static List<Pot> CalculatePots(List<PlayerBetInfo> bets) { /* ... */ }
        {
            ...
            var activeBets = bets.Where(b => /* ... */;
            bool hasAllIn = activeBets.Any(b => /* ... */;

            if (!hasAllIn)
            {
                int totalAmount = bets.Sum(b => /* ... */;
                if (totalAmount == 0)
                    return new List<Pot>();

                var eligibleIds = activeBets.Select(b => /* ... */;
            ...
            var remaining = bets.ToDictionary(b => /* ... */;
