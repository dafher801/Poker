...
namespace TexasHoldem.Director
{
    public class MockEventPublisher : MonoBehaviour, IGameEventBroadcaster
    {
        ...
        private IEnumerator RunMockScenario()
        {
            ...
            string[] playerNames = { "Player (You)", "Alice", "Bob", "Charlie" };
            ...
            Card[][] holeCards =
            {
                new[] { new Card(Suit.Spade, Rank.Ace), new Card(Suit.Heart, Rank.King) },
                new[] { new Card(Suit.Diamond, Rank.Queen), new Card(Suit.Club, Rank.Jack) },
                new[] { new Card(Suit.Heart, Rank.Ten), new Card(Suit.Spade, Rank.Nine) },
                new[] { new Card(Suit.Club, Rank.Eight), new Card(Suit.Diamond, Rank.Seven) }
            };
