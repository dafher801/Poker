...
namespace TexasHoldem.Director
{
    public class AiAutoMatchDirector : MonoBehaviour
    {
        ...
        private async void Start()
        {
            ...
            {
                Debug.LogError($"[AiAutoMatch] 자동 대전 중 예외 발생: {ex}");
            }
        }
        ...
        private async Task RunAutoMatchAsync()
        {
            ...
            Debug.Log($"플레이어 수: {PlayerCount}, 핸드 수: {_handCount}, 시작 칩: {_startingChips}, 블라인드: {_smallBlind}/{_bigBlind}");
            ...
            for (int i = 0; i < PlayerCount; i++)
            {
                ...
                players.Add(new PlayerData($"ai_{i}", $"AI_{i}({personality})", _startingChips, i));
            }
            ...
            Debug.Log($"초기 칩 총합: {initialTotalChips}");
            ...
            for (int hand = 1; hand <= _handCount; hand++)
            {
                ...
                if (activePlayers < 2)
                {
                    Debug.Log($"[핸드 {hand}] 활성 플레이어가 {activePlayers}명이므로 대전을 종료합니다.");
                    break;
                }
                ...
                {
                    Debug.LogError($"[핸드 {hand}] 칩 총합 불일치! 초기: {initialTotalChips}, 현재: {currentTotal}");
                }
        ...
        private void PrintChipStatus(List<PlayerData> players)
        {
            ...
            for (int i = 0; i < players.Count; i++)
            {
                ...
                sb.Append($"{players[i].Name}={status}");
            }
        ...
        {
            Debug.Log("\n=== 자동 대전 요약 ===");
            Debug.Log($"완료된 핸드 수: {_handsCompleted}");

            int totalActions = _totalFolds + _totalChecks + _totalCalls + _totalRaises + _totalAllIns;
            if (totalActions > 0)
            {
                Debug.Log($"총 액션 수: {totalActions}");
                Debug.Log($"  Fold: {_totalFolds} ({100f * _totalFolds / totalActions:F1}%)");
                Debug.Log($"  Check: {_totalChecks} ({100f * _totalChecks / totalActions:F1}%)");
                Debug.Log($"  Call: {_totalCalls} ({100f * _totalCalls / totalActions:F1}%)");
                Debug.Log($"  Raise: {_totalRaises} ({100f * _totalRaises / totalActions:F1}%)");
                Debug.Log($"  AllIn: {_totalAllIns} ({100f * _totalAllIns / totalActions:F1}%)");
            }
            ...
            Debug.Log($"칩 총합 보존: {(chipsPreserved ? "OK" : "FAIL")} (초기: {initialTotalChips}, 최종: {finalTotal})");

            // 최종 칩 순위
            var ranked = players.OrderByDescending(p => p.Chips).ToList();
            Debug.Log("최종 순위:");
            for (int i = 0; i < ranked.Count; i++)
            {
                Debug.Log($"  {i + 1}위: {ranked[i].Name} - {ranked[i].Chips} 칩");
            }
    ...
    internal class HandLogCollector
    {
        ...
        public void OnPlayerActed(PlayerActedEvent e)
        {
            string actionStr = e.ActionType switch
            {
                ...
                ActionType.Call => $"Call {e.Amount}",
                ActionType.Raise => $"Raise {e.Amount}",
                ActionType.AllIn => $"AllIn {e.Amount}",
                _ => e.ActionType.ToString()
            };

            _actionLogs.Add($"    좌석{e.SeatIndex}: {actionStr}");
        ...
        public void OnShowdownResult(ShowdownResultEvent e)
        {
            ...
            {
                string cards = string.Join(", ", entry.HoleCards.Select(c => c.ToString()));
                string result = entry.IsWinner ? "WIN" : "";
                sb.AppendLine($"    좌석{entry.SeatIndex}: [{cards}] {entry.Rank} {result}");
            }
        ...
        {
            var sb = new System.Text.StringBuilder();
            sb.Append($"  결과({e.Reason}): ");
            var awards = e.Awards.Select(a => $"좌석{a.SeatIndex}에 {a.Amount}칩({a.PotLabel})");
